...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flagSwitch_Total)
    {
        info->event[info->num] = EVENT_Switch_Total;
        info->num++;
        dataPoints->valueSwitch_Total = gizStandardDecompressionValue(Switch_Total_BYTEOFFSET,Switch_Total_BITOFFSET,Switch_Total_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagSwitch_Node)
    {
        info->event[info->num] = EVENT_Switch_Node;
        info->num++;
        dataPoints->valueSwitch_Node = gizStandardDecompressionValue(Switch_Node_BYTEOFFSET,Switch_Node_BITOFFSET,Switch_Node_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagSwitch_Custody)
    {
        info->event[info->num] = EVENT_Switch_Custody;
        info->num++;
        dataPoints->valueSwitch_Custody = gizStandardDecompressionValue(Switch_Custody_BYTEOFFSET,Switch_Custody_BITOFFSET,Switch_Custody_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagClean_Procedures)
    {
        info->event[info->num] = EVENT_Clean_Procedures;
        info->num++;
        dataPoints->valueClean_Procedures = gizStandardDecompressionValue(Clean_Procedures_BYTEOFFSET,Clean_Procedures_BITOFFSET,Clean_Procedures_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagCustody)
    {
        info->event[info->num] = EVENT_Custody;
        info->num++;
        dataPoints->valueCustody = gizStandardDecompressionValue(Custody_BYTEOFFSET,Custody_BITOFFSET,Custody_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
        
    if(0x01 == issuedData->attrFlags.flagTiming_Order_On)
    {
        info->event[info->num] = EVENT_Timing_Order_On;
        info->num++;
        dataPoints->valueTiming_Order_On = gizX2Y(Timing_Order_On_RATIO,  Timing_Order_On_ADDITION, exchangeBytes(issuedData->attrVals.valueTiming_Order_On));
    }
        
    if(0x01 == issuedData->attrFlags.flagTiming_Off)
    {
        info->event[info->num] = EVENT_Timing_Off;
        info->num++;
        dataPoints->valueTiming_Off = gizX2Y(Timing_Off_RATIO,  Timing_Off_ADDITION, exchangeBytes(issuedData->attrVals.valueTiming_Off));
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valueSwitch_Total != cur->valueSwitch_Total)
    {
        GIZWITS_LOG("valueSwitch_Total Changed\n");
        ret = 1;
    }
    if(last->valueSwitch_Node != cur->valueSwitch_Node)
    {
        GIZWITS_LOG("valueSwitch_Node Changed\n");
        ret = 1;
    }
    if(last->valueSwitch_Custody != cur->valueSwitch_Custody)
    {
        GIZWITS_LOG("valueSwitch_Custody Changed\n");
        ret = 1;
    }
    if(last->valueClean_Procedures != cur->valueClean_Procedures)
    {
        GIZWITS_LOG("valueClean_Procedures Changed\n");
        ret = 1;
    }
    if(last->valueCustody != cur->valueCustody)
    {
        GIZWITS_LOG("valueCustody Changed\n");
        ret = 1;
    }
    if(last->valueTiming_Order_On != cur->valueTiming_Order_On)
    {
        GIZWITS_LOG("valueTiming_Order_On Changed\n");
        ret = 1;
    }
    if(last->valueTiming_Off != cur->valueTiming_Off)
    {
        GIZWITS_LOG("valueTiming_Off Changed\n");
        ret = 1;
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    gizStandardCompressValue(Switch_Total_BYTEOFFSET,Switch_Total_BITOFFSET,Switch_Total_LEN,(uint8_t *)devStatusPtr,dataPoints->valueSwitch_Total);
    gizStandardCompressValue(Switch_Node_BYTEOFFSET,Switch_Node_BITOFFSET,Switch_Node_LEN,(uint8_t *)devStatusPtr,dataPoints->valueSwitch_Node);
    gizStandardCompressValue(Switch_Custody_BYTEOFFSET,Switch_Custody_BITOFFSET,Switch_Custody_LEN,(uint8_t *)devStatusPtr,dataPoints->valueSwitch_Custody);
    if(dataPoints->valueClean_Procedures >= Clean_Procedures_VALUE_MAX)
    {
        GIZWITS_LOG("[ERROR] valueClean_Procedures Error , Illegal Overstep\n");
        return -1;
    }
    else
    {
        gizStandardCompressValue(Clean_Procedures_BYTEOFFSET,Clean_Procedures_BITOFFSET,Clean_Procedures_LEN,(uint8_t *)devStatusPtr,dataPoints->valueClean_Procedures);  
    }
    if(dataPoints->valueCustody >= Custody_VALUE_MAX)
    {
        GIZWITS_LOG("[ERROR] valueCustody Error , Illegal Overstep\n");
        return -1;
    }
    else
    {
        gizStandardCompressValue(Custody_BYTEOFFSET,Custody_BITOFFSET,Custody_LEN,(uint8_t *)devStatusPtr,dataPoints->valueCustody);  
    }
    gizByteOrderExchange((uint8_t *)devStatusPtr->wBitBuf,sizeof(devStatusPtr->wBitBuf));


    devStatusPtr->valueTiming_Order_On = exchangeBytes(gizY2X(Timing_Order_On_RATIO,  Timing_Order_On_ADDITION, dataPoints->valueTiming_Order_On)); 
    devStatusPtr->valueTiming_Off = exchangeBytes(gizY2X(Timing_Off_RATIO,  Timing_Off_ADDITION, dataPoints->valueTiming_Off)); 
