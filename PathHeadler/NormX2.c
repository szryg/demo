...
#define NORMX2_T1 3000  //齿轮泵运行时长
#define NORMX2_T2 6000  //进水阀运行时长
#define NORMX2_T3 6000  //超声波
#define NORMX2_T4 30000  //放水超时
#define NORMX2_T5 10000  //达到限低后延迟关阀时间
#define NORMX2_REPEAT_TIMES 2    //重复次数



void NormX2(u8 in, u8 in2)    //如果流程刚启动，则in为1，否则为0   舱门 (key类) gate 
{
	...
	if(state == 2)
	{
		if((time() - Time_Past1) >= NORMX2_T1)
		{
			EXOUT_Close(SW_MOTOR);
			general_window_ShowInfo("等待注水完成");
			if(((time() - Time_Past1) >= NORMX2_T2) || high)  //等待60s 或者 到了限高 就关闭水阀 
			{
	...
	if(state == 4)
	{
	    if((time() - Time_Past2) >= NORMX2_T3)        // 20min
		{
			//关超声波
			EXOUT_Close(SW_WAVE);
			state = 5;
	...
	if(state == 6)
	{
		if(low == 1 || time() - Time_Past2 >=NORMX2_T4)   //到达限低   需要等待超时30分钟
		{
			state =7;
		    Time_Past1 = time();
	...
	if(state == 8)
	{
		if(repeat++ < NORMX2_REPEAT_TIMES)      //重复执行
		{
			state = 1;
			return;
		}
