...
#define AUTO_T1 3000  //齿轮泵运行时长
#define AUTO_T2 6000  //进水阀运行时长
#define AUTO_T3 6000  //超声波
#define AUTO_T4 30000  //放水超时
#define AUTO_T5 10000  //达到限低后延迟关阀时间

#define AUTO_T6 3000   //检测浊度时的放水时间

u8 AUTOX1_REPEAT_TIMES;    //重复次数
...
void Auto(u8 in, u8 in2)    //如果流程刚启动，则in为1，否则为0   舱门 (key类) gate 
{
	...
	if(state == 2)
	{
		if((time() - Time_Past1) >= AUTO_T1)
		{
			...
			if(((time() - Time_Past1) >= AUTO_T2) || high)  //等待60s 或者 到了限高 就关闭水阀 
			{
	...
	if(state == 6)
	{
		if(low == 1 || time() - Time_Past2 >=AUTO_T4)   //到达限低   需要等待超时30分钟
		{
			state =7;
		    Time_Past1 = time();
	...
	if(state == 8)
	{
		if(repeat++ < AUTOX1_REPEAT_TIMES)      //重复执行
		{
			state = 1;
			return;
		}
